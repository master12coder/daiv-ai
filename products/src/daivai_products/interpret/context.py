"""Knowledge context builder — loads lordship rules, gemstone logic, and scripture
...
from daivai_engine.scriptures import query as scripture_db


logger = logging.getLogger(__name__)

_KNOWLEDGE_DIR = (
    Path(__file__).parent.parent.parent.parent / "engine" / "src" / "daivai_engine" / "knowledge"
)

# Fallback: try to find knowledge dir relative to installed daivai_engine
...
def _get_knowledge_dir() -> Path:
    ...
    global _knowledge_dir_resolved
    if _knowledge_dir_resolved is not None:
        return _knowledge_dir_resolved

    # Try the relative path first
    if _KNOWLEDGE_DIR.exists():
        _knowledge_dir_resolved = _KNOWLEDGE_DIR
        return _knowledge_dir_resolved

    # Try to find via daivai_engine package
    try:
        import daivai_engine.knowledge as kmod
...
def reset_caches() -> None:
    ...
    global _lordship_cache, _gemstone_cache, _knowledge_dir_resolved
    _lordship_cache = None
    _gemstone_cache = None
    _knowledge_dir_resolved = None


def _load_yaml(filename: str) -> dict[str, Any]:
    """Load a YAML knowledge file."""
    path = _get_knowledge_dir() / filename
    if not path.exists():
