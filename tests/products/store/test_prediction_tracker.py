...
class TestPredictionTracker:
    ...
    def test_update_outcome(self, temp_tracker):
        pred = Prediction(
            chart_id=1, prediction_date="2026-01-01", category="health", prediction="Test"
        )
        pid = temp_tracker.log_prediction(pred)
    ...
    def test_invalid_outcome_raises(self, temp_tracker):
        pred = Prediction(
            chart_id=1, prediction_date="2026-01-01", category="test", prediction="Test"
        )
        pid = temp_tracker.log_prediction(pred)
    ...
    def test_get_predictions_filter(self, temp_tracker):
        temp_tracker.log_prediction(
            Prediction(chart_id=1, prediction_date="2026-01-01", category="career", prediction="A")
        )
        temp_tracker.log_prediction(
            Prediction(chart_id=1, prediction_date="2026-02-01", category="health", prediction="B")
        )
        temp_tracker.log_prediction(
            Prediction(chart_id=2, prediction_date="2026-03-01", category="career", prediction="C")
        )
    ...
    def test_accuracy_dashboard(self, temp_tracker):
        p1 = temp_tracker.log_prediction(
            Prediction(chart_id=1, prediction_date="2026-01-01", category="career", prediction="A")
        )
        p2 = temp_tracker.log_prediction(
            Prediction(chart_id=1, prediction_date="2026-02-01", category="career", prediction="B")
        )
        p3 = temp_tracker.log_prediction(
            Prediction(chart_id=1, prediction_date="2026-03-01", category="health", prediction="C")
        )
