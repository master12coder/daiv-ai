...
class TestLifeEventsDB:
    def test_add_chart(self, temp_db):
        chart = ChartRecord(
            name="Test", dob="15/08/1990", tob="06:30", place="Jaipur", lagna="Karka"
        )
    ...
    def test_get_events(self, temp_db):
        chart_id = temp_db.add_chart(ChartRecord(name="Test", dob="01/01/1990", tob="12:00"))
        temp_db.add_event(
            LifeEvent(chart_id=chart_id, event_date="2015-01-01", event_type="career")
        )
        temp_db.add_event(
            LifeEvent(chart_id=chart_id, event_date="2016-06-01", event_type="marriage")
        )
    ...
    def test_find_similar_events(self, temp_db):
        ...
        temp_db.add_event(
            LifeEvent(
                chart_id=cid1, event_date="2015-01-01", event_type="marriage", dasha_lord="Venus"
            )
        )
        temp_db.add_event(
            LifeEvent(
                chart_id=cid2, event_date="2018-06-01", event_type="marriage", dasha_lord="Venus"
            )
        )
