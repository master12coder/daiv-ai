...
from jyotish_engine.compute.chart import ChartData, get_house_lord, get_planets_in_house
...
def detect_other_yogas(chart: ChartData) -> list[YogaResult]:
    ...
    if _planet_in_kendra_from_reference(chart, "Jupiter", "Moon"):
        yogas.append(YogaResult(
            name="Gajakesari Yoga", name_hindi="गजकेसरी योग", is_present=True,
    ...
    if sun.sign_index == mercury.sign_index:
        yogas.append(YogaResult(
            name="Budhaditya Yoga", name_hindi="बुधादित्य योग", is_present=True,
            planets_involved=["Sun", "Mercury"], houses_involved=[sun.house],
    ...
    if moon.sign_index == mars.sign_index:
        yogas.append(YogaResult(
            name="Chandra-Mangal Yoga", name_hindi="चन्द्र-मंगल योग", is_present=True,
            planets_involved=["Moon", "Mars"], houses_involved=[moon.house],
    ...
    for h in DUSTHANAS:
        ...
        if p and p.house in DUSTHANAS and p.house != h:
            yogas.append(YogaResult(
                name="Vipreet Raj Yoga", name_hindi="विपरीत राज योग", is_present=True,
                planets_involved=[lord], houses_involved=[h, p.house],
    ...
    for planet_name in PLANETS:
        ...
        if p.dignity == "debilitated":
            ...
            if cancelled:
                yogas.append(YogaResult(
                    name="Neech Bhanga Raj Yoga", name_hindi="नीच भंग राज योग",
                    is_present=True, planets_involved=[planet_name],
    ...
    if not has_planet_adjacent:
        yogas.append(YogaResult(
            name="Kemdrum Yoga", name_hindi="केमद्रुम योग", is_present=True,
            planets_involved=["Moon"], houses_involved=[moon.house],
    ...
    if all(chart.planets[p].house in kendra_trikona for p in ["Jupiter", "Venus", "Mercury"]):
        yogas.append(YogaResult(
            name="Saraswati Yoga", name_hindi="सरस्वती योग", is_present=True,
    ...
    for p in planets_in_10:
        if p.name in {"Jupiter", "Venus", "Mercury", "Moon"}:
            yogas.append(YogaResult(
                name="Amala Yoga", name_hindi="अमल योग", is_present=True,
                planets_involved=[p.name], houses_involved=[10],
    ...
    benefics_in_adhi = [pn for pn in ["Jupiter", "Venus", "Mercury"]
                        if chart.planets[pn].house in adhi_houses]
    if len(benefics_in_adhi) >= 2:
        yogas.append(YogaResult(
            name="Adhi Yoga", name_hindi="अधि योग", is_present=True,
            planets_involved=benefics_in_adhi, houses_involved=adhi_houses,
    ...
    if p9 and _is_in_kendra(p9.house) and _is_in_own_or_exalted(lord_9, p9.sign_index):
        yogas.append(YogaResult(
            name="Lakshmi Yoga", name_hindi="लक्ष्मी योग", is_present=True,
            planets_involved=[lord_9], houses_involved=[9, p9.house],
